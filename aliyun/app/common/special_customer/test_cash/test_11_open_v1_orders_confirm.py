...
@allure.feature('v1_order_confirm')
class Test_open_v1_orders_confirm(object):

    @pytest.mark.parametrize("parameter",['{\'mchId\': \'$[config]merchant_id$\', \'nonceStr\': \'$[python]str(int(time.mktime(time.strptime(str(datetime.datetime.now()),"%Y-%m-%d %H:%M:%S.%f"))))$\', \'sign\': \'sha256\', \'codAmount\': \'$[python]random.randrange(100,5000001,100)$\', \'weight\': \'$[python]random.randint(1,30000)$\', \'length\': \'$[python]random.randint(1,100)$\', \'width\': \'$[python]random.randint(1,100)$\', \'height\': \'$[python]random.randint(1,80)$\'}'])
    @pytest.mark.parametrize("headers",["{'Accept-Language': 'ZH-CN', 'content-type': 'application/x-www-form-urlencoded', 'Charset': 'UTF-8'}"])
    @pytest.mark.parametrize("address",['/open/v1/orders/$open_v1_orders_1_0_0_["data"]["pno"]$/confirm'])
    @pytest.mark.run(order=11)
    def test_test_open_v1_orders_confirm(self, parameter,headers,address):
        ...
        _address = ['/open/v1/orders/$open_v1_orders_1_0_0_["data"]["pno"]$/confirm']
