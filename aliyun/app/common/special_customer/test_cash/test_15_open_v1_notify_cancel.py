...
@allure.feature('取消揽件任务')
class Test_open_v1_notify_cancel(object):

    @pytest.mark.parametrize("parameter",['{\'nonceStr\': \'$[python]str(int(time.mktime(time.strptime(str(datetime.datetime.now()),"%Y-%m-%d %H:%M:%S.%f"))))$\', \'sign\': \'sha256\', \'mchId\': \'$[config]merchant_id$\'}'])
    @pytest.mark.parametrize("headers",["{'content-type': 'application/x-www-form-urlencoded', 'Charset': 'UTF-8', 'Accept-Language': 'ZH-CN'}"])
    @pytest.mark.parametrize("address",['/open/v1/notify/$open_v1_notify_0_0_0_["data"]["ticketPickupId"]$/cancel', '/open/v1/notify/$open_v1_notify_1_0_0_["data"]["ticketPickupId"]$/cancel'])
    @pytest.mark.run(order=15)
    def test_test_open_v1_notify_cancel(self, parameter,headers,address):
        baseTest = BaseTestCase()
    
        _parameter = ['{\'nonceStr\': \'$[python]str(int(time.mktime(time.strptime(str(datetime.datetime.now()),"%Y-%m-%d %H:%M:%S.%f"))))$\', \'sign\': \'sha256\', \'mchId\': \'$[config]merchant_id$\'}']
        ...
        _address = ['/open/v1/notify/$open_v1_notify_0_0_0_["data"]["ticketPickupId"]$/cancel', '/open/v1/notify/$open_v1_notify_1_0_0_["data"]["ticketPickupId"]$/cancel']
