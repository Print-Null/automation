...
class Test_bi_qery_list(object):
    ...
    def test_test_bi_qery_list(self):
        ...
        url8 = self.host + 'parcelloseapi/taskList?'+self.iframe_0+'&'+self.iframe_2+'&'+self.iframe_3+'&'+self.iframe_1+'&page=1&pagesize=100&keyword='+self.keyword8+'&source=&state=&tab=1&startTime=&endTime=&area=&vip_type=&taskStartTime='+str(int(time.mktime(time.strptime(str(datetime.date.today()) + " 01:00:00","%Y-%m-%d %H:%M:%S"))))+'&taskEndTime='+str(int(time.mktime(time.strptime(str(datetime.date.today() +datetime.timedelta(days=1)) + " 00:59:59","%Y-%m-%d %H:%M:%S"))))+'&handleStartTime=&handleEndTime='
        url9 = self.host + 'parcelloseapi/taskList?'+self.iframe_0+'&'+self.iframe_2+'&'+self.iframe_3+'&'+self.iframe_1+'&page=1&pagesize=100&keyword='+self.keyword9+'&source=&state=&tab=1&startTime=&endTime=&area=&vip_type=&taskStartTime='+str(int(time.mktime(time.strptime(str(datetime.date.today()) + " 01:00:00","%Y-%m-%d %H:%M:%S"))))+'&taskEndTime='+str(int(time.mktime(time.strptime(str(datetime.date.today() +datetime.timedelta(days=1)) + " 00:59:59","%Y-%m-%d %H:%M:%S"))))+'&handleStartTime=&handleEndTime='
        resp = requests.post(url=url, headers=headers, verify=False)
        resp8 = requests.post(url=url8, headers=headers, verify=False)
        resp9 = requests.post(url=url9, headers=headers, verify=False)
        ...
        RedisBase().set("7bi_qery_list_pno",resp.json()["data"]["row"][-1]["pno"], ex=6000)
        RedisBase().set("7bi_qery_list_task_id",resp.json()["data"]["row"][-1]["task_id"], ex=6000)

        RedisBase().set("8bi_qery_list_pno", resp8.json()["data"]["row"][-1]["pno"], ex=6000)
        RedisBase().set("8bi_qery_list_task_id", resp8.json()["data"]["row"][-1]["task_id"], ex=6000)

        RedisBase().set("9bi_qery_list_pno", resp9.json()["data"]["row"][-1]["pno"], ex=6000)
        RedisBase().set("9bi_qery_list_task_id", resp9.json()["data"]["row"][-1]["task_id"], ex=6000)
