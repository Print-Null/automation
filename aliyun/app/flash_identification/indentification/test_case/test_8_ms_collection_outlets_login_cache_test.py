...
@allure.feature('揽收网点经理登入MS')
class Test_ms_collection_outlets_login(object):

    @pytest.mark.parametrize("parameter",["{'login': '$[config]indentifi_manager_usr$', 'password': '$[config]indentifi_manager_pwd$'}"])
    ...
    def test_test_ms_collection_outlets_login(self, parameter,headers,address):
        ...
        _parameter = ["{'login': '$[config]indentifi_manager_usr$', 'password': '$[config]indentifi_manager_pwd$'}"]
