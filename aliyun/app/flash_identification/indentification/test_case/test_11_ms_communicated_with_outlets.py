...
class Test_ms_communicated_with_outlets(object):
    ...
    @pytest.mark.parametrize("parameter",['{\'states\': [], \'pno\': \'$courier_write_order_1_0_0_["data"]["parcel_info"]["pno"]$\', \'page_size\': 100, \'page_num\': 1, \'start_time\': \'$[python]datetime.date.today()$\', \'end_time\': \'$[python]datetime.date.today()$\'}'])
    ...
    def test_test_ms_communicated_with_outlets(self, parameter,headers,address):
        ...
        _parameter = ['{\'states\': [], \'pno\': \'$courier_write_order_1_0_0_["data"]["parcel_info"]["pno"]$\', \'page_size\': 100, \'page_num\': 1, \'start_time\': \'$[python]datetime.date.today()$\', \'end_time\': \'$[python]datetime.date.today()$\'}']
        ...
        RedisBase().set('01ms_communicated_with_outlets_' + str(_parameter.index(parameter)) + '_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["items"][0]["id"]', resp.json()["data"]["items"][0]["id"], ex=6000)
