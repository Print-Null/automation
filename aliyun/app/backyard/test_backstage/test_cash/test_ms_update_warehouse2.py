...
class Test_Ms_Update_Warehouse2():
    def setup(self):
    ...
    def test_ms_warehouse2(self):
        url_te = self.host + "ms/api/setting/store/manager/TH01510101"
        # url_tra = self.host + "ms/api/setting/store/manager/TH01280201"
        url_tra = self.host + "ms/api/setting/store/manager/TH47150103"
        # self.ms_session = read_ini("bi_login","ms_token","ms_token")
        self.ms_session = RedisBase().get("ms_token")
        header = {
        ...
        if self.runenv_py == 'trunk':
            data_te = json.dumps(read_json_file("ms_update_warehouse2_te")) #测试环境仓管
            ...
            data_tra = json.dumps(read_json_file("ms_update_warehouse2_tra"))  # 预发布环境仓管
