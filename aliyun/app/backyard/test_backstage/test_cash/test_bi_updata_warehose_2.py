...
@allure.feature("修改申请加班车仓管员账号2")
class Test_Bi_Update_warehouse_2():
    ...
    def setup(self):
        ...
        self.Warehouse_man_personInfo_end_login = ReadConfig().get_config(self.runenv_py, "Warehouse_man_personInfo_end_login")
    @pytest.mark.run(order=4)
    def test_bi_update_warehouse_2(self):
        null = None
        ...
        data_te = json.dumps(read_json_file("apply_warehouse2_te")) #测试环境仓管
        print(data_te)
        print(type(data_te))
        ...
        if self.runenv_py == "trunk":
            ...
            assert_that(resp.json()["body"]["staff_info_id"]).is_equal_to(int(self.Warehouse_man_personInfo_end_login))
        elif self.runenv_py == "training":
            data_tra = json.dumps(read_json_file("apply_warehouse2_tra")) #测试环境仓管
            resp_tra = requests.post(url=url, headers=header, data=data_tra, verify=False)
            print(resp_tra.json())
            assert_that(resp_tra.json()["code"]).is_equal_to(0)
            assert_that(resp_tra.json()["body"]["staff_info_id"]).is_equal_to(int(self.Warehouse_man_personInfo_end_login))
