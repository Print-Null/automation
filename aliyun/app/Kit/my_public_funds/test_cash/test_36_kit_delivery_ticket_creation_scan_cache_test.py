...
@allure.feature('到付运费-kit快递员登录-对退货单号进行交接')
class Test_kit_delivery_ticket_creation_scan(object):
    ...
    @pytest.mark.parametrize("address",['api/courier/v1/parcels/$ms_store_parcel_v0_#headers#_0_["data"]["parcel_info"]["returned_pno"]$/delivery_ticket_creation_scan?isFromScanner=false'])
    @pytest.mark.run(order=36)
    def test_test_kit_delivery_ticket_creation_scan(self, parameter,headers,address):
        ...
        _address = ['api/courier/v1/parcels/$ms_store_parcel_v0_#headers#_0_["data"]["parcel_info"]["returned_pno"]$/delivery_ticket_creation_scan?isFromScanner=false']
