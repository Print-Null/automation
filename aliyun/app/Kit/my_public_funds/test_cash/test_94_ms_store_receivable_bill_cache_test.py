...
@allure.feature('ms-财务人员登录-快递员回款-确认回款')
class Test_ms_store_receivable_bill(object):

    @pytest.mark.parametrize("parameter",["{'exclude_ids': []}"])
    ...
    @pytest.mark.parametrize("address",['ms/api/store/receivable_bill/$ms_store_receivable_bill_#headers#_["data"]["items"][0]["id"]$/exclude'])
    @pytest.mark.run(order=94)
    def test_test_ms_store_receivable_bill(self, parameter,headers,address):
        baseTest = BaseTestCase()
    
        _parameter = ["{'exclude_ids': []}"]
        ...
        _address = ['ms/api/store/receivable_bill/$ms_store_receivable_bill_#headers#_["data"]["items"][0]["id"]$/exclude']
        ...
        RedisBase().set('ms_store_receivable_bill_' + str(_parameter.index(parameter)) + '_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["bill_info"]["unpaid_amount"]', resp.json()["data"]["bill_info"]["unpaid_amount"], ex=6000)
