...
@allure.feature('到付运费-ms-寄件网点-网点经理登录-待网点沟通-1')
class Test_ms_store_pickup_diff_tickets(object):

    @pytest.mark.parametrize("parameter",['{\'states\': [0, 2], \'pno\': \'$kit_pickups_parcel_0_#headers#_0_["data"]["parcel_info"]["pno"]$\', \'page_size\': 20, \'page_num\': 1, \'start_time\': \'\', \'end_time\': \'\'}'])
    ...
    def test_test_ms_store_pickup_diff_tickets(self, parameter,headers,address):
        baseTest = BaseTestCase()
    
        _parameter = ['{\'states\': [0, 2], \'pno\': \'$kit_pickups_parcel_0_#headers#_0_["data"]["parcel_info"]["pno"]$\', \'page_size\': 20, \'page_num\': 1, \'start_time\': \'\', \'end_time\': \'\'}']
        ...
        RedisBase().set('ms_store_pickup_diff_tickets_' + str(_parameter.index(parameter)) + '_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["items"][0]["id"]', resp.json()["data"]["items"][0]["id"], ex=6000)
