...
@allure.feature('MS-10000登录-线路管理-新建常规线路-获取公司名称')
class Test_ms_api_fleet(object):
    ...
    def test_test_ms_api_fleet(self,headers,address):
        ...
        for item in resp.json()["data"]["items"]:
            if item["van_count"]>0:
                RedisBase().set("fleetId",item["id"],ex=6000)
                RedisBase().set("company_name", item["name"], ex=6000)
                break
        ...
        # logging.info("jsonschema文件path:../data/jsonschema/18_get_company_name.json")
        # with open("../data/jsonschema/18_get_company_name.json", "r", encoding = "utf-8") as f:
        #     shcema = json.load(f)
