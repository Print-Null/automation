...
@allure.feature('仓管员问题单提交-获取破损后图片信息')
class Test_kit_v1_parcels_aft(object):
    ...
    @pytest.mark.parametrize("address",['api/courier/v1/parcels/difficult_upload_photo?fileName=aft_%2$[config]difficult_upload_photo$'])
    @pytest.mark.run(order=182)
    def test_test_kit_v1_parcels_aft(self,headers,address):
        ...
        _address = ['api/courier/v1/parcels/difficult_upload_photo?fileName=aft_%2$[config]difficult_upload_photo$']
        ...
        RedisBase().set('kit_v1_parcels_aft_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["object_key"]', resp.json()["data"]["object_key"], ex=6000)
