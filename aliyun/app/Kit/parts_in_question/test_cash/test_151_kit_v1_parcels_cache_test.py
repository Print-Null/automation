...
@allure.feature('仓管员问题单提交-获取图片信息')
class Test_kit_v1_parcels(object):
    ...
    @pytest.mark.parametrize("address",['api/courier/v1/parcels/difficult_upload_photo?fileName=$[config]difficult_upload_photo$'])
    ...
    def test_test_kit_v1_parcels(self,headers,address):
        ...
        _address = ['api/courier/v1/parcels/difficult_upload_photo?fileName=$[config]difficult_upload_photo$']
        ...
        RedisBase().set('kit_v1_parcels_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["object_key"]', resp.json()["data"]["object_key"], ex=6000)
