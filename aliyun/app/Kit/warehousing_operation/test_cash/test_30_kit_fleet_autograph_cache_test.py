...
@allure.feature('仓管员-车辆出港-获取装载实况照片')
class Test_kit_fleet_autograph(object):
    ...
    @pytest.mark.parametrize("address",['api/courier/v1/fleet/autograph/upload_photos?fileName=$[config]fileName$'])
    @pytest.mark.run(order=30)
    def test_test_kit_fleet_autograph(self,headers,address):
        ...
        _address = ['api/courier/v1/fleet/autograph/upload_photos?fileName=$[config]fileName$']
        ...
        RedisBase().set('kit_fleet_autograph_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["object_key"]', resp.json()["data"]["object_key"], ex=6000)
