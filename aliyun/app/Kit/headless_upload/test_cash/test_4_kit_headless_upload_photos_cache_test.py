...
@allure.feature('新增无头件包裹-获取包裹图片')
class Test_kit_headless_upload_photos(object):
    ...
    @pytest.mark.parametrize("address",['api/courier/v1/headless/upload_photos?fileName=$[config]fileName$'])
    @pytest.mark.run(order=4)
    def test_test_kit_headless_upload_photos(self,headers,address):
        ...
        _address = ['api/courier/v1/headless/upload_photos?fileName=$[config]fileName$']
        ...
        RedisBase().set('kit_headless_upload_photos_' + str(_headers.index(headers)) + '_' + str(_address.index(address)) + '_["data"]["object_key"]', resp.json()["data"]["object_key"], ex=6000)
        ...
        # logging.info("jsonschema文件path:../data/jsonschema/4_headless_upload_photos.json")
        # with open("../data/jsonschema/4_headless_upload_photos.json", "r", encoding = "utf-8") as f:
        #     shcema = json.load(f)
